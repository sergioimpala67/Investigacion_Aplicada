...
alpha_y = float(sys.argv[3])*pi/180.	#alpha_rot
alpha_z = float(sys.argv[4])*pi/180.	#alpha_tilt
Dist = float(sys.argv[5])  #distancia_entre_proteina_sensor:2,3,4,5
if len(sys.argv)>6:
    name = sys.argv[6]
else:
    name = ''
...
translation = array([ctr[0], ymin-Dist, ctr[2]]) # 2 Angs over the x-z plane
